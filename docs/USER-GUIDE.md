...
limit while earning a good risk-adjusted return — a backward-looking proxy, not a forecast. Below it sit the backtests as **compact, horizontally-scrollable tables** (max drawdown / CAGR / Calmar shown as *buy-&-hold → timed*, with a per-crash column), all running the **exact live F+C Thrust ladder** (the same `v23.mjs` rule the regime uses), no look-ahead, turnover-costed:
- **F+C Thrust backtest (the live design)** — the ladder on **deep benchmark history** (SPY/QQQ/SOXX, back through 2000/2008/2020/2022) vs buy-&-hold. Per instrument: did it **cut** max drawdown (✓/✗), **hold** the −35% mandate, **improve** Calmar (✓/✗), at what CAGR cost, and a per-crash list (⚠ = a crash it didn't cut). This is where the *timing* edge is actually tested against real bears — it's methodology evidence on proxies, not your book.
- **Combo — F+C Thrust × the scarcity book** — the timing overlay on **your** target-weighted basket vs buy-&-hold. It reports a **realistic** number: **only the IRA sleeve is timed** (the taxable sleeve is buy-and-hold for timing — it changes only on scarcity/thesis decisions), so the realistic drawdown-cut is ~the IRA share of the fully-timed cut (shown in `[brackets]`). **Caveat:** the book's history is short/bull-only, so the tail benefit only shows through a real drawdown.
- **Combo through real bears** — because the book is too young, the build-out sleeve is mapped to **long-history liquid analogues** (SMH/XLI/XLU/XME/ITA/PHO/XLK) and the same ladder is run through **2008/2020/2022**, again **IRA-only timed** (taxable buy-&-hold). This is where the combo is actually cycle-tested. It's an **analogue** — composition/weights approximate, mapping chosen with hindsight, survivors — so read the **drawdown-cut and Calmar delta**, not absolute CAGR. Full caveats: `docs/DRAWDOWN-DEFENSE.md` → *Backtest realism*.
- The **selection edge** (does the scarcity alpha actually pick winners) is graded separately by the **Track record** (live, unbiased) and the **cross-sectional signal backtest** (rank IC + hit-rate, an upper bound with survivorship caveats).

See `docs/DRAWDOWN-DEFENSE.md` and `docs/ARCHITECTURE.md`.
...
scarcity **crowded** the next scan shrinks that name's weight → a **smaller buy** here. A name held across
more than one account is the **sum** of its per-account signal rows (so a split position isn't undercounted).
Diversifier names keep their funding weights; offline it falls back to plan weights.
