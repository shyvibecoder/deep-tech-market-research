...
const HELP = {
  ...
  attribution: { title: "Factor attribution — alpha or just beta?", body: `\n    <p>The honesty gate's teeth. A high hit-rate or a rising basket is <em>not</em> proof of skill — the book could simply be loaded on factors anyone can buy. So we regress the basket's daily return on a small set of <strong>tradeable factors: market (SPY), momentum (MTUM), and — crucially — a thematic proxy (QQQ).</strong> The <strong>intercept is the residual alpha</strong>: the return left over <em>after</em> market, momentum, and the AI/tech theme are accounted for. Including the theme leg is the whole point — without it, this single-factor AI-capex book's beta would masquerade as alpha.</p>
