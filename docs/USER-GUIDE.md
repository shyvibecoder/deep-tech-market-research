...
limit while earning a good risk-adjusted return — a backward-looking proxy, not a forecast. A **trend-brake backtest** line below it shows, on this basket and with no look-ahead, whether a moving-average brake actually cut max-drawdown and improved Calmar vs. buy-and-hold (the dial's premise, tested). The brake now runs at the **live 200-day MA** and the line includes the **turnover cost** charged for switching in/out of cash (so its Calmar isn't overstated). When the basket lacks enough history to test the 200-day brake honestly, the line reads **"unproven"** instead of showing a number. A separate **"Brake proof"** block then runs the *same* 200-day brake on **deep history pulled from the accumulated database** (SPY/QQQ/SOXX, back through 2000/2008/2020/2022) so the dial's tail claim is tested against real crashes — showing, per instrument, whether the brake **cut** max drawdown (✓/✗), **improved** Calmar (✓/✗), its CAGR cost, and a per-crash list where ⚠ marks a crash the brake **whipsawed** instead of cutting. Finally, the **"System backtest (as designed)"** block runs the *full* timing overlay (200-day brake **+** breadth fast-entry — re-risk when ≥60% of names reclaim their 20-DMA, turnover-costed) on the **book's own names**, three ways — **buy-&-hold vs +brake vs +brake+fast-reentry** — and checks each against the **−35% mandate** (✓ holds / ✗ breaches), so you can see exactly what the timing layer does to both drawdown and CAGR. Selection is held fixed (the alpha selection signal is graded separately by the Track record + cross-sectional signal backtest). The proofs are methodology evidence on deep history, *not* a promise about your exact future book. See `docs/DRAWDOWN-DEFENSE.md` and `docs/ARCHITECTURE.md`.
