...
limit while earning a good risk-adjusted return — a backward-looking proxy, not a forecast. Below it sit three backtest lines, all running the **exact live F+C Thrust ladder** (the same `v23.mjs` rule the regime uses), no look-ahead, turnover-costed:
