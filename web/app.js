...
function renderMetrics() {
  ...
  const scFast = fr ? `<p class="foot"><strong>Fast re-entry proof</strong> — the breadth fast-entry overlay (re-risk a notch when ≥${Math.round(fr.breadth_threshold * 100)}% of ${fr.names.length} names reclaim their 20-DMA) vs a plain 200-DMA brake, on the book's own deep history (${fr.years}y): CAGR ${(fr.plain.cagr * 100).toFixed(1)}%→<strong>${(fr.fast.cagr * 100).toFixed(1)}%</strong> (${fr.cagr_gain >= 0 ? "+" : ""}${(fr.cagr_gain * 100).toFixed(1)}pts), maxDD −${(fr.plain.max_drawdown * 100).toFixed(0)}%→−${(fr.fast.max_drawdown * 100).toFixed(0)}% (${fr.maxdd_cost >= 0 ? "+" : ""}${(fr.maxdd_cost * 100).toFixed(0)}pts), Calmar ${num(fr.plain.calmar)}→${num(fr.fast.calmar)}, time-in-market ${Math.round(fr.time_in_market_plain * 100)}%→${Math.round(fr.time_in_market_fast * 100)}% → <strong class="${fr.worth_it ? "pos" : "neg"}">${fr.worth_it ? "worth it" : "not worth it"}</strong>.</p>` : "";
