...
const HELP = {
  ...
  triggers: { title: "Deploy / exit triggers", body: `
    ...
    <li><strong>Trim rule</strong> (auto) — a name &gt;2× cost basis AND &gt;50× <strong>forward</strong> P/E → trim ⅓. Forward P/E is a <em>best-effort keyless</em> fetch (Yahoo gates it, so it's often unavailable); set <code>forward_pe</code> per position in <code>positions.local.json</code> to guarantee the rule can evaluate. It also needs your cost basis. <strong>No forward-P/E value → the P/E condition isn't met (no trim)</strong>; there is no trailing-P/E fallback.</li>
  ...
  regime: { title: "Timing posture (regime)", body: `
    <img src="img/regime-ladder.svg" alt="The F+C Thrust ladder: first match wins — CRASH_OFF (252-day return below 0 AND 60-day vol above 25%) → DEFENSIVE; else TREND (composite above its 200-DMA) → RISK-ON; else THRUST (close above a rising 20-DMA below trend) → NEUTRAL, the fast re-entry; else → DEFENSIVE. An exit-only composite-stress overlay (VIX-term inversion AND HY-velocity top-5%) forces DEFENSIVE." style="width:100%;height:auto;border:1px solid var(--line);border-radius:10px;margin:8px 0"/>
  ...
  digest: { title: "Agent digest", body: `
    <p>An optional <strong>analyst + red-team</strong> read of <em>what changed this scan</em> for the <strong>deep-tech build-out</strong> sleeve: fresh quotes (incl. forward P/E when available), recent SEC filings (8-K/10-Q items — backlog, capacity, guidance, pricing), news headlines, and whether any deploy/exit <strong>trigger</strong> looks closer.</p>
